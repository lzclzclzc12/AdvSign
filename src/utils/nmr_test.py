...
class NMR(object):
    ...
    def forward_mask(self, vertices, faces):
        ...
        masks = self.masks.cpu().detach().numpy()
        return masks
    #
...
def example():
    obj_file = '../carassets/sign4.obj'
    data_path = '../data2/phy_attack/train/'
    img_save_dir = 'D:/PyCharmProject/Full-coverage-camouflage-adversarial-attack-gh-pages/src/data2/phy_attack/render_test_res/'
    
    vertices, faces = neural_renderer.load_obj(obj_file)
    
    texture_mask = np.zeros((faces.shape[0], 6, 6, 6, 3), 'int8')
    with open('../carassets/sign_faces.txt', 'r') as f:
    ...
    faces_var = torch.autograd.Variable(torch.from_numpy(faces[None, :, :].cpu().numpy()).cuda(device=0))
    vertices_var = torch.from_numpy(vertices[None, :, :].cpu().numpy()).cuda(device=0)
    # Textures
    texture_size = 2
    textures = np.load('../logs/epoch-20+batch_size-1+lr-0.01+model-resnet50+loss_func-loss_midu+loss_content+loss_smooth+lamb-0.0001+D1-0.9+D2-0.1+T-0.0001+/texture.npy')
    ...
    data_lsit = os.listdir(data_path)
    for data_path1 in data_lsit:
        data_path1 = os.path.join(data_path, data_path1)
        ...
        print(np.max(im_rendered), np.min(im_rendered) , np.min(img) , np.max(img))
        # scipy.misc.imsave(img_save_dir + 'test_render1.png', im_rendered)
        # scipy.misc.imsave(img_save_dir + 'test_origin1.png', img)
        break
