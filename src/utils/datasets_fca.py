...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, faces, texture_size, vertices, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # textures = np.random.random((1, faces.shape[0], texture_size, texture_size, texture_size, 3)).astype('float32')
        ...
        self.mask_renderer = nmr.NeuralRenderer(img_size=640).to(self.device)
        self.mask_renderer.renderer.renderer.camera_mode = "look"
        self.mask_renderer.renderer.renderer.light_direction = [1, 0, 1]
        self.mask_renderer.renderer.renderer.camera_up = [0, 0, 1]
        self.mask_renderer.renderer.renderer.background_color = [1 , 1 , 1]

        self.mask_renderer1 = nmr.NeuralRenderer(img_size=640).to(self.device)
        self.mask_renderer1.renderer.renderer.camera_mode = "look"
        self.mask_renderer1.renderer.renderer.light_direction = [0, 0, 1]
        self.mask_renderer1.renderer.renderer.camera_up = [0, 0, 1]
        self.mask_renderer1.renderer.renderer.background_color = [1, 1, 1]

        # self.mask_renderer.renderer.renderer.light_intensity_directional = 0.0
        # self.mask_renderer.renderer.renderer.light_intensity_ambient = 1.0
    ...
    def __getitem__(self, index):
        ...
        img, (h0, w0), (h, w), (veh_trans, cam_trans) , path1 = load_image1(self, index)
        # cam_trans = [[2 , 0 , 0] , [0 , 180 , 0]]
        # cam_trans[0][2] += 1
        # cam_trans[0][0] += 3.
        # cam_trans[0][2] -= 3
        # cam_trans[0][2] -= 1
        ...
        # 正面
        cam_trans1 = [[2 , 0 , 0] , [0 , 0 , 0]]
        veh_trans1 = [[0 , 0 , 0] , [0 , 0 , 0]]
        # path2 = int(path1[4:][:-4])
        # if path2 >= 200 and path2 < 300:
        #     cam_trans[0][1] += 3
        # elif path2 < 100 or path2 >= 300:
        #     cam_trans[0][1] -= 3
        # cam_trans[0][2] -= 1
        # cam_trans[0][2] -= 0.5
        # cam_trans = [[2 , 0 , 0] , [0 , 180 , 0]]
        ...
        eye1, camera_direction1, camera_up1 = nmr.get_params(cam_trans1, veh_trans1)
        self.mask_renderer1.renderer.renderer.eye = eye1
        self.mask_renderer1.renderer.renderer.camera_direction = camera_direction1
        self.mask_renderer1.renderer.renderer.camera_up = camera_up1

        # render image via neural renderer   self.vertices_var, self.faces_var 这是车的模型参数   imgs_pred: R(M , T_adv , θ)

        # imgs_pred2 = self.mask_renderer1.forward(self.vertices_var, self.faces_var, self.textures)
        imgs_pred = self.mask_renderer.forward(self.vertices_var, self.faces_var, self.textures)
        imgs_pred1 = self.mask_renderer1.forward(self.vertices_var, self.faces_var, self.textures)  # texture的正面


        # Image.fromarray(np.uint8(255 * imgs_pred1.squeeze().cpu().data.numpy().transpose(1, 2, 0))).show()


        imgs_pred = imgs_pred / torch.max(imgs_pred)
        # imgs_pred = cv2.resize(imgs_pred , (self.img_size, self.img_size))

        # load mask, note that for simplicity, we get the mask via applying segmentation on the rendered image (i.e., imgs_pred)
        if self.ret_mask:  # yes
        ...
        if nl:
            labels[:, 1:5] = xyxy2xywhn(labels[:, 1:5], w=ratio[0] * w, h=ratio[1] * h)  # xyxy to xywh normalized
        ...
        # Applying mask, the transformation function in paper   img: φ(R(M , T_adv , θ))
...
def load_image(self, index):
    ...
    path = sb.join(path.rsplit(sa, 1)).rsplit('.', 1)[0] + '.npz'    #  data1/phy_attack/train/data0.npz
...
def load_image1(self, index):
    ...
    if r != 1:  # if sizes are not equal
        img = cv2.resize(img, (int(w0 * r), int(h0 * r)),
                         interpolation=cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR)
    return img, (h0, w0), img.shape[:2], (veh_trans, cam_trans) , path1  # img, hw_original, hw_resized
