...
mask_id = ['a' , 'b' , 'c' , 'd' , 'e' , 'f' , 'g' , 'h' , 'i' , 'l' , 'm' , 'n'
            'o' , 'p' , 'q' , 'r' , 's' , 't' , 'u' , 'v' , 'w' , 'x' , 'y' , 'z']


# texture_content = torch.from_numpy(np.load('logs/new_mm/mask_a.npy')).cuda(device=0)
# texture_canny = torch.from_numpy(np.load('logs/new_mm/mask_a.npy')).cuda(device=0)
# texture_canny = (texture_canny >= 0.7).int()
D1 = 0.95
D2 = 0.05


texture_content = torch.from_numpy(np.load('logs/new_mm/black.npy')).cuda(device=0)
texture_canny_list = []

for i in os.listdir(r'D:\PyCharmProject\Full-coverage-camouflage-adversarial-attack-gh-pages\src\logs\new_mm'):
    if i[0] == 'm':
        texture_canny1 = torch.from_numpy(np.load(os.path.join(r'D:\PyCharmProject\Full-coverage-camouflage-adversarial-attack-gh-pages\src\logs\new_mm' , i))).cuda(device=0)
        texture_canny1 = (texture_canny1 >= 0.7).int()
        texture_canny_list.append(texture_canny1)



def loss_content_diff(tex , texture_canny , texture_content):
...
def train(hyp, opt, device):
    ...
    texture_param = np.load('logs/new_mm/black.npy')
    # texture_param = np.load('./nr_examples/texture.npy')
    # texture_param = np.load('logs/text.npy')
    # texture_param = np.random.random((1, faces.shape[0], texture_size, texture_size, texture_size, 3)).astype('float32')

    # texture_param[0] = texture_origin.cpu().detach().numpy()
    # texture_param = texture_origin.cpu().detach().numpy()

    # texture_param = np.random.random((1, faces.shape[0], texture_size, texture_size, texture_size, 3)).astype('float32')
    texture_param = torch.autograd.Variable(torch.from_numpy(texture_param).to(device), requires_grad=True)
    # neural_renderer.save_obj(r'D:\PyCharmProject\Full-coverage-camouflage-adversarial-attack-gh-pages\src\nr_examples\test\test.obj', vertices, faces, texture_param)
    ...
    for epoch in range(1, epochs+1):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, texture_img, masks, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            texture_canny = texture_canny_list[index]

            # compute loss
            loss1, loss_items = compute_loss(train_out, targets.to(
                device))
            loss3 = loss_content_diff(texture_param , texture_canny , texture_content)
            loss2 = loss_smooth(texture_img, masks)
            # loss = loss1 + loss2
            print('loss1: {:5f} , loss2: {:5f} , loss3: {:5f}'.format(loss1.item() , loss2.item() , 1e-3 * loss3.item()))
            loss = loss1 + 1e-3 * loss3 + loss2
            # Backward
            optim.zero_grad()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='carassets/carla.yaml', help='data.yaml path')
    parser.add_argument('--lr', type=float, default=0.01, help='learning rate for texture_param')
    parser.add_argument('--obj_file', type=str, default='carassets/new_mm/mask/black.obj', help='3d car model obj')
    parser.add_argument('--faces', type=str, default='carassets/new_mm/new_mm_faces.txt',
                        help='exterior_face file  (exterior_face, all_faces)')
    parser.add_argument('--datapath', type=str, default='./data2/phy_attack/',
                        help='data path')
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run  no
